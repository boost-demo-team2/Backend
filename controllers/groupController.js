...
const getGroupByPublicStatus = async (req, res) => {
  try {
    ...
    if (isPublic) {
      ...
      return res
        .status(200)
        .json({ message: "공개 그룹 조회 성공", data: groups });
    }
...
const checkGroupAccess = async (req, res) => {
  try {
    ...
    // 그룹 비밀번호 조회(권한 확인)
    const sql =
      "SELECT password FROM `groups` WHERE groupId = ? AND isPublic = false";
    const [groupRows] = await db.promise().execute(sql, [groupId]);

    if (!groupRows || groupRows.length === 0) {
      return res
        .status(404)
        .json({ message: "해당 비공개 그룹을 찾을 수 없습니다." });
    }
...
const updateGroup = async (req, res) => {
  try {
    const { groupId, password, groupName, image, description, isPublic } =
      req.body;
    ...
    return res
      .status(200)
      .json({ message: "그룹 정보가 성공적으로 수정되었습니다." });
  } catch (error) {
    console.error(error);
    return res
      .status(500)
